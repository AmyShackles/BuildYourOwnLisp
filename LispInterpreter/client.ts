...
import {Terminal} from "xterm/lib/public/Terminal.js";
...
document.getElementById("dispose").addEventListener(
    "click", disposeRecreateButtonHandler);
  ...
  addDomListener(actionElements.findNext, "keypress", e => {
    if (e.key === "Enter") {
      ...
      const searchOptions = {
        ...
        wholeWord :
            (document.getElementById("whole-word") as HTMLInputElement).checked,
        caseSensitive :
...
function runFakeTerminal(): void {
  ...
  term._core.register(
      term.addDisposableListener("paste", (data, ev) => { term.write(data); }));
}
...
function initOptions(term: TerminalType): void {
  ...
  booleanOptions.forEach(o => {
    html += `<div class="option"><label><input id="opt-${o}" type="checkbox" ${
        term.getOption(o) ? "checked" : ""}/> ${o}</label></div>`;
  });
  html += '</div><div class="optin-group">';
  numberOptions.forEach(o => {
    html += `<div class="option"><label>${o} <input id="opt-${
        o}" type="number" value="${term.getOption(o)}"/></label></div>`;
  });
  html += '</div><div class="option-group">';
  Object.keys(stringOptions).forEach(o => {
    if (stringOptions[o]) {
      html += `<div class="option"><label>${o} <select id="opt-${o}">${
          stringOptions[o]
              .map(v => `<option ${term.getOption(o) === v ? "selected" : ""}>${
                       v}</option>`)
              .join("")}</select></label></div>`;
    } else {
      html += `<div class="option"><label>${o} <input id="opt-${
          o}" type="text" value="${term.getOption(o)}"/></label></div>`;
    }
  });
  html += "</div>";
...
function addDomListener(element: HTMLElement, type: string,
...
function updateTerminalSize(): void {
  const cols = parseInt(
      (<HTMLInputElement>document.getElementById(`opt-cols`)).value, 10);
  const rows = parseInt(
      (<HTMLInputElement>document.getElementById(`opt-rows`)).value, 10);
  const width = (cols * term._core.renderer.dimensions.actualCellWidth +
